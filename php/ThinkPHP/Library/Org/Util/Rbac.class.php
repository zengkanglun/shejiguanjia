...
//                echo 'end';exit;
    ...
    public static function getAccessList($authId)
    {
        ...
        $table['node'] . " as node " .
        "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  )   and node.level=1 limit 1";
//        $table['node']. " as node " .
//            "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id";
       // echo $sql;
        $apps   = $db->query($sql);
        ...
        foreach ($apps as $key => $app) {
            ...
            $table['access'] . " as access ," .
            $table['node'] . " as node " .
            "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) )  and access.node_id=node.id and node.level=2 and node.pid={$appId} ";
//                $table['node'] . " as node " .
//                "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$appId} and node.status=0";
            ...
            foreach ($modules as $key => $module) {
                ...
                if ('PUBLIC' == strtoupper($moduleName)) {
                    ...
                    $table['access'] . " as access ," .
                    $table['node'] . " as node " .
                    "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) )  and access.node_id=node.id and node.level=3 and node.pid={$moduleId}";
//                        $table['node'] . " as node " .
//                        "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$moduleId} and node.status=0";
                    $rs = $db->query($sql);
                    foreach ($rs as $a) {
